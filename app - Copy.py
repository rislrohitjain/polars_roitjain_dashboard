...
if active_bytes is not None:
    try:
        ...
        # =====================================================================
        # 🟢 START OF INJECTED SEMANTIC SEARCH OVERLAY (ISOLATED ELEMENT)
        # =====================================================================
        from api.semantic_search import run_isolated_semantic_filter
        
        with st.expander("🤖 AI Semantic Prompt Query Canvas (Isolated)", expanded=False):
            st.markdown("<div class='section-watermark'>AI Language Translation Node Layer</div>", unsafe_allow_html=True)
            user_prompt = st.text_input(
                "Filter data using natural language statements:", 
                placeholder="e.g., Show rows where status is active and amount is above 1000",
                key="semantic_search_input_field"
            )
            
            if user_prompt:
                with st.spinner("Local AI processing filter requirements..."):
                    raw_df = run_isolated_semantic_filter(user_prompt, raw_df)
                    st.caption(f"✨ Semantic filter executed. Downstream pipeline now processing {raw_df.height} rows.")
        # =====================================================================
        # 🔴 END OF INJECTED SEMANTIC SEARCH OVERLAY
        # =====================================================================
